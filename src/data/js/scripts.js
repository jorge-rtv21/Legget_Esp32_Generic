...
const traeHost = () => {
    if (window.location.hostname == 'localhost') {
        return "192.168.1.17"; // TODO: Modo Desarrollo IPv4 del Equipo en su red
    } else {
...
const OnOffRelay = (relay, command) => {
    ...
    ws.send(`{"protocol": "relay", "output": "${relay}", "value": ${command} }`);
}
...
//Funciones del sensor de huella
function fP_add() {
    ws.send("fP_add");
}
...
function validarFormulario(e) {
    ...
    switch (e.target.name) {
        ...
        case "id":
            ...
            /*Colores*/
        case "black_R":
            validarRango(e.target, e.target.name, 0, 9999);
            break;
        case "black_G":
            validarRango(e.target, e.target.name, 0, 9999);
            break;
...
const validarPassword = () => {
    ...
    // Interceptar el Evento Submit Solo en los Formularios (wifi-mqtt-device-admin-color)
if (pathname == "/esp-wifi" || pathname == "/esp-mqtt" || pathname == "/esp-device" || pathname == "/esp-admin" || pathname == "/esp-color") {
...
function manejadorValidacion(e) {
    ...
    } else if (page == "Estado del Sensor de color") {
        if (campos.id) {
            document.getElementById('formulario_mensaje').classList.remove('formulario_mensaje-activo');
            SweetAlert('¿Guardar?', page, 'question', this);
        } else {
...
const uploadSettings = () => {
        ...
        =============================================*/
        if (file != undefined) {
            if (file["type"] != "text/json" && file["name"] != "settingwifi.json" && file["name"] != "settingmqtt.json" && file["name"] != "settingrelays.json" && file["name"] != "settingadmin.json" && file["name"] != "settingcolors.json") {
